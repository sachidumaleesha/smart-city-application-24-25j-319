...
const AccidentDetectionPage = () => {
    ...
    const sendWhatsAppAlert = async (camera: string, frameBase64: string) => {
        ...
        if (currentTime - lastAlertTime.current < ALERT_INTERVAL) {
            console.log(`⏳ Cooldown active: Skipping WhatsApp alert (Remaining: ${(ALERT_INTERVAL - (currentTime - lastAlertTime.current)) / 1000}s)`);
            return;
        }

        if (!camera || !frameBase64) {
            console.error("❌ Missing required parameters for WhatsApp Alert", { camera, frameBase64 });
            return;
        }
    ...
    useEffect(() => {
        const fetchAlerts = async () => {
            try {
                const response = await fetch("http://localhost:5000/api/accidentDetection/alerts");
                const data = await response.json();
    ...
    //         <h1 className="text-3xl font-bold mb-4">🚨 Real-Time Multi-Camera Accident Detection</h1>

    //         <div className="grid grid-cols-2 gap-4">
    //             {cameras.map((camera) => (
    //                 <div key={camera} className="border rounded-lg overflow-hidden shadow-lg">
    //                     <h2 className="text-center font-semibold bg-gray-200 py-2">{camera}</h2>
    //                     <img
    ...
    //                             <img src={`data:image/jpeg;base64,${alert.frame}`} className="mt-2 w-64 rounded" alt="Accident Frame" />
    //                         </li>
    //                     ))}
    //                 </ul>
    //             </div>
