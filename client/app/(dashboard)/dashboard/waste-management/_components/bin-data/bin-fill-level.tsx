...
export default function BinFillLevel({
  ...
  useEffect(() => {
    if (distance !== undefined) {
      ...
      const percentage = Math.round(
        ((binHeight - clampedDistance) / binHeight) * 100
      );
      ...
      if (newAlertStatus && !alertSent) {
        sendTelegramAlert(binName, percentage);
        setAlertSent(true);
      }
  ...
  const sendTelegramAlert = async (binName: string, fillPercentage: number) => {
    try {
      const response = await fetch("/api/waste-management/telegram", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
    ...
    <Card className={`w-full ${isAlert ? "border-red-400" : ""}`}>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="relative h-48 w-32 mx-auto border-2 border-gray-300 rounded-md overflow-hidden">
          ...
          >
            <div className="absolute right-0 w-4 h-4 -mt-2 mr-1 text-black">
              ↑
            </div>
          </div>
        </div>
